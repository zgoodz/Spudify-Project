...
function Search({ makePlaylist, setTopTracks}) {
    ...
    if(searchInput.onDropDown === "Title") {
        url=`https://api.spotify.com/v1/search?q=${searchInput.onSearch}&type=track&market=US&limit=24`
    } else if (searchInput.onDropDown === "Artist") {
        url=`https://api.spotify.com/v1/search?q=${searchInput.onSearch}&type=artist&market=US&limit=24`
    } else {
        url =`https://api.spotify.com/v1/browse/new-releases?country=US&limit=24`
    }
